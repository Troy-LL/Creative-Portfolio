...
Done when: fall → settle → cursor OK → click flips to back → scroll flattens → fold to end pose with hatch cover slide in sync and the WebM window visible in the slit. Flat rest reads as one sheet (no panel seams, no paper outline halo).
...
- Markup: `index.html` hatch slot (video + vignette + cover)
